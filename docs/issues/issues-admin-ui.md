...
| [Issue 010](#issue-010-ダッシュボード画面-改善) | ダッシュボード画面（改善） | P2 | 1日 | ✅ 完了 | 2026-02-05 |
| [Issue 011](#issue-011-発送一覧画面-ux改善) | 発送一覧画面（UX改善） | P2 | 1.5日 | ✅ 完了 | 2026-02-05 |
| [Issue 012](#issue-012-発送詳細画面-情報拡充) | 発送詳細画面（情報拡充） | P2 | 1日 | ✅ 完了 | 2026-02-05 |

**注**: Issue番号は既存のまま維持（0xx範囲外だが、学習テスト用のため変更不要）

**合計工数**: Phase 1: 2.5日（✅ 完了）、Phase 2: 3.5日（✅ 完了）
...
- [x] RETURNED カードが警告表示される
- [x] 要対応発送リストが正しく表示される（最大5件）
- [x] 要対応発送が存在しない場合、リストは非表示
- [x] 「すべて表示」リンクが正常に動作する
...
### 受け入れ条件
- [x] 行クリックで発送詳細サイドパネルが開く
- [x] フィルタ適用中の条件がチップで表示される
- [x] ソート（注文ID, ステータス, 更新日時）が動作する
- [x] チェックボックスで複数選択できる
- [x] ステータス一括変更ができる（部分成功対応）
- [x] CSVエクスポートができる
...
#### 4. 操作エリア
- **ステータス変更はプルダウン選択方式**（誤操作防止のため）
  - デフォルト値: 現在のステータス
  - 選択可能: 次のステータスのみ（遷移ルールに従う）
  - 変更ボタンで確定（プルダウン選択だけでは変更されない）
- **ステータス遷移ルール**:
  - CREATED → READY
  - READY → SHIPPED（配送業者・追跡番号入力必須）
  - SHIPPED → DELIVERED / RETURNED
  - DELIVERED → RETURNED
- 返送処理（RETURNED選択時は赤色の警告ボタン表示）
- 再出荷（将来拡張）
...
- [x] 追跡番号がコピーできる
- [x] 外部追跡リンクが正常に動作する
- [x] ステータス変更がプルダウン選択方式で動作する
- [x] 選択可能なステータスが遷移ルールに従っている
- [x] 返送処理が正常に動作する
