...
# Validate target app
case "$TARGET_APP" in
    admin-next-shadcn|admin-antd-pro|admin-horizon-ui|admin-mui|admin-square-ui|admin-ui|all)
        echo "Building: $TARGET_APP"
        ;;
    *)
        echo "Error: Unknown target app: $TARGET_APP"
        echo "Usage: $0 [admin-next-shadcn|admin-antd-pro|admin-horizon-ui|admin-mui|admin-square-ui|admin-ui|all]"
        exit 1
        ;;
esac
